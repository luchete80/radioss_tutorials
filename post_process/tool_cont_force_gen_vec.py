...
mesh = 1 #0 to 2
tool = 0 #1 is 4 tools and 2 is 8 tools

# Sphere nodes 386 nodes + center = 387

#(220/2mm)+1 = 111
#111 x 111 = 12321
##################################################

mesh_tool_nodes = np.zeros([3, 3, 2], dtype=int)
#MESH 0
mesh_tool_nodes[0] = [[12321,12708], #+387*1
                      [12321,13869],
                      [12321,15417]]
#MESH 1
mesh_tool_nodes[1] = [[21609,21996], #147*147
                      [21609,23157],
                      [21609,24705]]

#MESH 2
mesh_tool_nodes[2] = [[48841,49228],
                      [48841,50389],
                      [48841,51937]]
...
def write_list(force):
  fi_x = open("force_m"+str(mesh)+"_t_"+str(tool)+".csv","w")
  fi_x.write("t,fx,fy,fz\n")

  dt = 1.0e-3
  t = 0.0
  for i in range(len(force)):
    fi_x.write(str(t) + ", " + str(force [i,0]) + ", "+str(force [i,1])+", "+str(force [i,2])+"\n" )
    print(force [i,0],force [i,1],force [i,2])
    t +=dt
...
def open_files_with_extension(directory, extension):
    ...
    force = []
    print ("Force length", len(files))
    force = np.zeros((len(files),3))

    force_y = [];force_y = np.zeros(len(files))
    ...
    for file_name in files:
      ...
      try:
          with open(file_path, 'r') as f:
            ...
            print ("idx %d" %idx) 
            lines = f.readlines()
            print ("file " + str(j) + "/"+ str(tot) + ", Array loc ", idx)
            if (first):  
            ...
            for k in range (node_count) :
              ...
              #print (k+line_ini)
              if ( k > mesh_tool_nodes[mesh,tool,0] and k <mesh_tool_nodes[mesh,tool,1]):
                #print (lines[line_ini+k]+"\n")
                force_tool_z += narr[2]
                for d in range (3) :
                  force[idx][d] += narr[d]                


            print ("Contact Force Sum: ", force_z, ", Contact Force in tool Node Range: ", force[idx])
