      ...
      for j in range (divisions+1):
        ...
        for i in range (divisions+1):
          ...
          for k in range (n): #CHECK IF THERE IS AN EXISTENT NODE THERE
            if ( abs(x - self.nodes[k][0])<1.0e-6 and abs(y - self.nodes[k][1])<1.0e-6 and abs(z - self.nodes[k][2])<1.0e-6 ):
...
class Part:
  ...
  def printRadioss(self,f): 
    ...
    #cambio a locales:
    mesh0     = self.mesh[0]
    ini_id    = mesh0.ini_elem_id
    elnod     = mesh0.elnod
    elem_cnt  = mesh0.elem_count
    #n_per_elt = elnod.shape[1]
    start     = start_id

    buffer = []
      
    for i in range(elem_cnt):
        # calculamos la lista de valores sumando 'start' a cada nodo
        vals = [i + ini_id] + [v + start for v in elnod[i]]
        # construimos la línea en un solo join usando rjust
        line = ''.join(str(v).rjust(10) for v in vals)
        buffer.append(line + '\n')
        
        #vals   = [i + ini_id] + list(elnod[i] + start)
        #buffer.append(''.join(str(v).rjust(10) for v in vals) + '\n')
        if i % 500 == 0:
            print(line)
    ...
    for i in range (len(self.inter)):
      ...
      if (not self.thermal):
        ...
        if (self.inter[i].id_master<=self.multi_tool_N+start_part): #was 4
          ...
          f.write(writeIntField(istf,10)+writeIntField(ithe,10)+writeIntField(igap,10)+"                   0         0"+ writeIntField(icurv,10)+"         0\n")
          
      f.write("#          Fscalegap             GAP_MAX             Fpenmax\n")
      f.write(writeFloatField(fscalegap,20,6)+"                   0                   0\n")
