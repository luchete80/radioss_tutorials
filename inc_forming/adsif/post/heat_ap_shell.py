    ...
    for j, file_name in enumerate(files, 1):
        ...
        try:
            with open(file_path, 'r') as f:
                ...
                # Leer fuerzas de contacto y calcular áreas
                contact_nodes = set()
                
                cn = 0
                for k in range(ini_node, end_node):
                    if force_line_ini + k < len(lines):
                        #force_data = list(map(float, lines[force_line_ini + k].split()))
                        force_data = [float(num) for num in lines[force_line_ini+k].split()]
                        force_magnitude = math.sqrt(force_data[0]**2 + force_data[1]**2 + force_data[2]**2)
                        
                        if ( node_coords[k][0]>(-L/2.0 + largo_supp) and node_coords[k][0]<(L/2.0-largo_supp) and 
                             node_coords[k][1]>(-L/2.0 + largo_supp) and node_coords[k][1]<(L/2.0-largo_supp) ):
                          if force_magnitude > 0.0:
                              contact_nodes.add(k)
                              print ("Node", k)
                              cn +=1
                print("Contact nodes: ", cn)
                
                # Calcular áreas para nodos superiores en contacto
                top_contact_elements = set()
                nod_area = 0.0 #For comparison
                for node in contact_nodes:
                    for elem_id in node_elements[node]:
                        top_contact_elements.add(elem_id)
                    ...
                    cont_heat[idx] += area * (T_t - float(lines[temp_line_ini + node]) ) 
                    nod_area += area
                cont_heat[idx] *= h_tool
                
                top_area = 0.0
                for elem_id in top_contact_elements:
                    #print("Elem area: ", calculate_element_area(node_coords, elem_id, element_nodes, node_coords))
                    top_area += calculate_element_area(node_coords, elem_id, element_nodes, node_coords)
